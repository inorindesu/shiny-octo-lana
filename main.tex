...
2011年12月\marginpar{這段展示了陰謀論}，Accuvant安全測試公司在由Google資助的一項研究的研究報告中指出，Internet Explorer 9安全性已相當接近同期的Google Chrome 15，反觀同期的Mozilla Firefox 9則居於其後[39]，但隨後有NSS Labs安全測試公司指此項研究有失中立性，並指出Accuvant公司曾接受Google的巨額贊助，在研究過程中Firefox的許多安全特性也被刻意忽略而未作測試，選取測試用惡意外掛模組的方式亦有偏袒Chrome打壓Firefox之嫌[40]。
...
Firefox使用者可以透過安裝擴充套件來新增或修改Firefox的功能\marginpar[（左文）如右，這裡說明了Firefox最棒的功能]{（右文）如左，這裡說明了Firefox最棒的功能}。擴充套件的種類包羅萬象：像滑鼠手勢、廣告視窗阻擋、加強的分頁瀏覽等等。擴充套件雖然提供了高度自由化的擴充功能，但是使用者可能在尋找和安裝擴充套件上遭遇困難，也會建議把擴充套件的功能整合到Firefox中，像是分頁瀏覽就是從一個Mozilla上的附加元件MultiZilla中移植過來的。[44]
...
\stepcounter{footnote}
\footnotetext{我的語無倫次是為了讓這張圖可以擁有三個註腳XD然而，這樣設置註腳會造成註腳可能和圖不在同一頁}
